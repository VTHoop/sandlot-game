...
## What this is
**Sandlot** (working name; brand "Sandlot ___" TBD) — a turn-based, async-first multiplayer baseball game. Core mechanic: a **hidden-number duel** — the pitcher submits a secret number (1–999), the batter submits one, and the server resolves the outcome by how close they are (circular distance on a ring of 999; see ADR-0016), with player **attributes (1–5)** resizing the outcome bands. Wrapped in a **salary-cap league** where you draft real, stat-derived MLB players plus one custom "signature" player. A game is a **6-inning regulation** (a deliberate product rule — shorter than baseball's 9 for casual async play; walk-off and extra-innings rules apply at the 6th; see ADR-0017). First milestone: a **6-team family-league beta**. It's also a **public portfolio piece** — optimize for *visible judgment* (the ADRs), and **ship the vertical slice**.
