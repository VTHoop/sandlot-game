...
- **Difference** — the circular distance between the two numbers, folded into `0–500` (closer guess → smaller difference → better outcome for the batter). This single value is what the bands partition.
- **Outcome bands** — the `0–500` difference line is partitioned into an **ordered stack of outcomes, best → worst**. Whichever band the difference lands in is the result of the at-bat. A band's **width** comes from a table lookup keyed by the matchup (see *attribute differentials*); wider band = more likely outcome.
...
- **Back half — `FO / PO / GB / K`.** An **elastic remainder**: K is anchored at the far end (`500`), GB stretches to fill the gap, and FO/PO split the leftover. Because it's interdependent rather than a simple sum, it's verified by **Monte Carlo simulation** against public MLB rate baselines rather than by exact-value unit tests.
