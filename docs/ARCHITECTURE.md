...
| `packages/engine/src/atBat/` | Top-level resolver: `foldDifference` (ring-999 circular fold), `classify` (outcome + elastic GB band), `applyOutcome` (identity-preserving one-base advancement over a runner-aware `BaseState`, ADR-0018), `groundBall/` (the GB sub-resolution — eligibility × speed-driven sub-band partition → `GroundBallResult`, with third-out run suppression, SAN-16/ADR-0019), and `resolveAtBat` — the dual-use authoritative resolution (ADR-0016) |
