...
| `convex/users.ts` | Account provisioning (SAN-55/ADR-0023): `provision` — the client-called, argument-free mutation that turns a Clerk session into the `users` row every gated function resolves against. `upsertUserBySubject` is the only place a `users` row is created (the dev seed calls it too); `displayName` is derived from the Clerk identity once and never re-synced |
| `convex/seed.ts` | Dev-only fixture seed (SAN-54): `seedDevGame` mints a scheduled game `startGame` can open — one synthetic owner, two clubs, ten players a side, two lineups. Double-fenced: an `internalMutation` (unreachable from any client) gated on `SANDLOT_DEV_SEED=true` (fail-closed). Additive per run — the roster is reused, a new game appended |
