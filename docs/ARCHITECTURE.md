...
**Resolution model:** the RangeFinder — takes attribute differentials, looks up each outcome band's width from a table, and assembles them into a 0–499 partition. Band order: `HR → 3B → 2B → 1B → IF1B → BB → FO → PO → GB/GO → K`. Front half (HR→BB) is a cumulative sum of direct table lookups. Back half (FO/PO/GB/K) is an elastic remainder. The top-level `resolveAtBat` (`src/atBat/`) folds the two committed numbers into a 0–499 difference (circular distance on a ring of 999, ADR-0016), classifies the band, and applies standard one-base advancement to produce the complete at-bat outcome. The `GB` band sub-resolves further (SAN-16/ADR-0019): gated by base state + outs and sized by the speed−awareness axis, it partitions into a ground-out / fielder's-choice / double-play / triple-play family (`GroundBallResult`), with third-out run suppression. The `1B` / `2B` / `FO` / `IF1B` bands sub-resolve their *runner movement* (SAN-17/ADR-0021, `src/atBat/advancement/`): extra-base advancement on a hit (low end of the band, range widening with runner speed + hitter power, no passing the lead runner), deep-fly/sac-fly on a fly out (runner on 3rd scores and 2nd tags to 3rd, < 2 outs), and the forced/2-out infield single. All deterministic in the folded space — no RNG, no schema change, and `emit-grid` is unaffected since band rates are unchanged.
...
| `convex/validators.ts` | Shared `v.union` field validators (outcome bands, the nullable GB `groundBallResult` sub-result, role, position, game status, half, 1–5 rating, runner-aware base state — `Id<'players'>`-or-null per base, attribute blocks); `outcomeBand` and `groundBallResult` are compile-time-locked to `@sandlot/engine` |
...
| `packages/engine/src/atBat/` | Top-level resolver: `foldDifference` (ring-999 circular fold), `classify` (outcome + elastic GB band), `applyOutcome` (identity-preserving one-base advancement over a runner-aware `BaseState`, ADR-0018), `groundBall/` (the GB sub-resolution — eligibility × speed-driven sub-band partition → `GroundBallResult`, with third-out run suppression, SAN-16/ADR-0019), `advancement/` (extra-base/deep-fly/sac-fly + forced/2-out infield single, SAN-17/ADR-0021), and `resolveAtBat` — the dual-use authoritative resolution (ADR-0016) |
