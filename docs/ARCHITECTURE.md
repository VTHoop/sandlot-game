...
| `packages/engine/src/atBat/` | Top-level resolver: `foldDifference` (ring-999 circular fold), `classify` (outcome + elastic GB band), `applyOutcome` (identity-preserving per-band primitive advancement — each band moves runners its own way — over a runner-aware `BaseState`, ADR-0018), `groundBall/` (the GB sub-resolution — eligibility × speed-driven sub-band partition → `GroundBallResult`, with third-out run suppression, SAN-16/ADR-0019), `advancement/` (extra-base/deep-fly/sac-fly + forced/2-out infield single, SAN-17/ADR-0021), and `resolveAtBat` — the dual-use authoritative resolution (ADR-0016) |
